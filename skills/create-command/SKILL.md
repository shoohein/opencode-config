...
Create a new OpenCode command as a `.md` file from the user's natural language description, validating it against 6 command design principles.
...
- `description` (required): a concise, unique, declarative sentence (1-100 chars, 120 max). The agent reads this first when deciding whether to use the command.
...
For each command found, collect its `name` (filename without `.md` extension), `description`, and full file path. Present the collected list to the user as a reference.
...
| 3 | **Explicit target** | If delegating, write the delegate name as natural language text in the body (e.g., `task-done スキルを使って完了処理してください`). Do not rely on frontmatter, external mappings, or naming conventions. |
| 4 | **Visible emptiness / Arguments are data** | If `$ARGUMENTS` is used, wrap it in delimiters. Default: `「$ARGUMENTS」`. Alternatives: `"$ARGUMENTS"`, `[$ARGUMENTS]`. Delimiters ensure arguments are treated as quoted data, not execution instructions. |
| 5 | **Extract complexity** | If the command needs logic beyond a simple launch instruction, extract to a skill and delegate to it instead. |
...
Before running checks, build an inventory of available skills (scan `skills/` and global skill directories) and known tools (`session_status`, `task_move`, `task_create`, `task_list`, etc.).
...
| 4 | Visible emptiness / Arguments are data | `$ARGUMENTS` used without delimiters | **Error**: wrap in `「$ARGUMENTS」` or equivalent — delimiters ensure arguments are treated as data, not instructions |

> Principle 6 (Extract complexity) is violated whenever Principle 1 is violated — complex logic detected in the body should be extracted to a skill.
...
- User explicitly overrides a validation error → respect their decision and create the file as requested. Log a note about the override.
