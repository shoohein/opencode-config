...
Run three checks at session end, report issues, and delegate fixes to existing commands/skills.

Checks:

1. **User requests vs actual work** — cross-reference the session conversation and `todowrite` state for unaddressed items
2. **Git commit status** — detect uncommitted or untracked files
3. **Unexpected file changes** — detect files modified outside the session's scope
...
Read `todowrite` state and review the session conversation for any requests the user made that have not been addressed.
...
### Step 3: Check unexpected file changes
...
- **No unexpected files** → proceed to Step 4.
- **Suspicious files found** → present them to the user with a note about why each seems out of scope. Suggest `git restore <file>` or ask the user to confirm the file is intentional. After the user responds, proceed to Step 4.
