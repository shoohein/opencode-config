...
- `description` (frontmatter): a concise sentence that supplements the agent name. The agent reads this first when deciding whether to invoke the agent. Choose words for information contrast over grammatical flow.
...
- Shared coding values (KISS, YAGNI, DRY, etc.) belong in the `coding-principles` skill, not in individual agent files. Both implementers and reviewers load this skill to align on the same values.
...
For each agent found, collect its `name`, `description`, and file path. Present the collected list to the user as a reference.
...
- **`name`**: Must match regex `^[a-z0-9]+(-[a-z0-9]+)*$`, 1-30 chars, and be **unique** across all agent files found in Step 1.
...
- **`output`**: What the agent produces. Describe the expected format semantically. The format must be rigid and machine-parseable (JSON, fixed markdown structure, etc.) to ensure composability with downstream tools or agents. No greetings, preambles, or free-form prose in the output. Example: "List of findings as a JSON array of {location, issue, severity}, with no wrapper text."
...
If the agent requires domain-specific instructions that must always be loaded when the agent runs, design one or more additional sections beyond the required elements. Keep the abstraction level of each section consistent with the others — do not mix high-level general rules with specific technical guidelines in the same list.
...
- Name collision detected (same name within the target location) → stop with an error. Do not overwrite.
