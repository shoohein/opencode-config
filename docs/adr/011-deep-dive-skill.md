...
- **代替案:**
  - Plan agent の常駐指示への埋め込み — 軽量タスクにオーバーヘッドがかかり、議論促進の文脈が不要なケースでノイズになるため不採用とした。
  - 自動ロード — トリガー判断が曖昧であり、「ただの計画」と「deep-dive」の区別はエージェントに任せるよりユーザーに任せる方が確実なため不採用とした。
  - v0.1 縮小版（theorist/pragmatist 召喚のみ） — 議論促進の本質が抜け落ち、アドバイザー召喚だけでは批判的対話の仕組みが欠けるため不採用とした。
- **トレードオフ:**
...
- **代替案:** ADR の Decision Frame を流用 — ADR は決定の記録であり、deep-dive が作るのは実装計画である。candidates / steps の違いが重要であることから不採用とした。
...
- **代替案:** 反証なしの単純な質疑応答 — Plan agent の既存機能と差別化されず、deep-dive の存在意義が薄れるため不採用とした。
...
- **代替案:** ADR または設計書を自動生成 — スキルが複数の責務を持つことになり、ADR 001 の原則（skills as commands: スキルは単一責務のコマンドである）に反するため不採用とした。
