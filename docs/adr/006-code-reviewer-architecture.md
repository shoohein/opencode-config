...
- **代替案:**
  - 機械的計測を独立 reviewer として分離する
    - **不採用理由:** 決定論的ツール（linter 等）でも実現可能であり、AI にやらせる根拠がないため
...
- **代替案:**
  - code-reviewer を cross-module 対応とする
    - **不採用理由:** 評価項目が局所的であるため architecture-reviewer との責務境界が曖昧になるため
...
- **代替案:**
  - code-testing-reviewer にテスト実行権限を与える
    - **不採用理由:** テスト実行は決定論的プロセスであり環境依存・副作用を伴うため
