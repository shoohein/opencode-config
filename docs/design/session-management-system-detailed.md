...
外部システム不在時は縮退する（§7 参照）。タスク管理システム（`tasks/`）は未実装のため、関連機能は縮退として扱う。
...
| 正規表現   | `/^【(?:完了\|中断\|回収)】\s*/`       |
| 基数       | 常に単一（重複不可）                   |
| 意味       | 表示ラベル。システムの制御状態ではない |
| `【完了】` | 正常終了したセッション                 |
| `【中断】` | ユーザーが中断したセッション           |
| `【回収】` | システムが自動起動した回収セッション   |

制御状態とは、システムが分岐・スキップ・再起動の判断に直接参照するデータを指す。接頭辞はその判断に使用しない。制御判断の情報源は OpenCode の `time.updated` と `SessionStatus`（SSoT）である。
...
- イベント再帰発火防止: 接頭辞が欠落している場合のみ書き込む（冪等）
...
4. **git notes 付与**（タスク管理システムのコミットがある場合）: `session_add_note(commit, { session: "<slug>" })` を実行
5. **セッションタイトル更新**:
...
**注意**: 回収セッションでは WIP コミットを作成しない。未コミット変更の保護はユーザーの次回セッション時に `git status` で認識される。

#### 4.2.1 放置セッションの検出アルゴリズム
...
1 回のスイープで回収するのは 1 セッションのみ。複数放置がある場合は次回以降のユーザーセッションで順次処理される。
...
| `SessionStatus` が `idle` または 404 かつ `now - started_at <= ttl_ms` | 正常完了直後とみなし、スキップする（クラッシュであれば TTL 超過後に次回セッションで検出されるため、安全側に倒れている） |
...
- ファイルが削除されても、`【完了】`/`【中断】` 接頭辞が付与された既回収セッションは保護されるため安全側に倒れる
